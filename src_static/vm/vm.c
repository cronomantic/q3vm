...
static const vmHeader_t* VM_LoadQVM(const uint8_t* bytecode,
                                    int length);
...
static char* VM_Indent();
/** For profiling, find the symbol behind this value */
static vmSymbol_t* VM_ValueToFunctionSymbol(int value);
static const char* VM_ValueToSymbol(int value);
/** Load a .map file for the virtual machine. The .map file
 * should have the same path as the .qvm file. */
static void VM_LoadSymbols(void);
...
static void VM_StackTrace(int programCounter, int programStack);
#endif
...
int VM_Create(const char* name, const uint8_t* bytecode, int length)
{
    Q_strncpyz(vm.name, name, VM_MAX_QPATH);
    const vmHeader_t* header = VM_LoadQVM(bytecode, length);
    if (!header)
    {
    ...
    if (!vm.compiled)
    {
        if (VM_PrepareInterpreter(header) != 0)
        {
            VM_Free();
            return -1;
        }
                                    ...
                                    int length)
{
    ...
    vm.dataBase  = (uint8_t*)Com_malloc(vm.dataAlloc, VM_ALLOC_DATA_SEC);
...
void VM_Free(void)
{
    ...
    if (vm.codeBase)
    {
        Com_free(vm.codeBase, VM_ALLOC_CODE_SEC);
        vm.codeBase = NULL;
    }

    if (vm.dataBase)
    {
        Com_free(vm.dataBase, VM_ALLOC_DATA_SEC);
        vm.dataBase = NULL;
    }

    if (vm.instructionPointers)
    {
        Com_free(vm.instructionPointers,  VM_ALLOC_INSTRUCTION_POINTERS);
...
void* VM_ArgPtr(intptr_t vmAddr)
{
    if (!vmAddr)
    {
        return NULL;
...
int VM_MemoryRangeValid(intptr_t vmAddr, size_t len)
{
    if (!vmAddr)
    {
        return -1;
...
static int VM_PrepareInterpreter(const vmHeader_t* header)
{
    ...
    vm.codeBase = (uint8_t*)Com_malloc(
        vm.codeLength * 4, VM_ALLOC_CODE_SEC); /* we're now int aligned */
    if (!vm.codeBase)
    {
...
#ifdef DEBUG_VM
    profileSymbol = VM_ValueToFunctionSymbol(0);
...
#define DISPATCH() goto nextInstruction
    ...
    nextInstruction2:
            ...
            if (programCounter < 0) /* system call */
            {
                ...
                *(int*)&image[programStack + 4] = -1 - programCounter;
                ...
                if (sizeof(intptr_t) != sizeof(int))
                {
                    ...
                    r = Com_systemCalls(argarr);
                }
                else
                {
                    r = Com_systemCalls((intptr_t*)&image[programStack + 4]);
                }
                ...
                *(int*)&image[programStack + 4] = stomped;
                ...
                {
                    Com_Printf("%s%i<--- %s\n", VM_Indent(), opStackOfs,
                               VM_ValueToSymbol(programCounter));
                }
#endif
            }
        ...
        goto_OP_ENTER:
            ...
            programStack -= v1;
#ifdef DEBUG_VM
            profileSymbol = VM_ValueToFunctionSymbol(programCounter);
            /* save old stack frame for debugging traces */
            *(int*)&image[programStack + 4] = programStack + v1;
            if (vm_debugLevel)
            {
                Com_Printf("%s%i---> %s\n", VM_Indent(), opStackOfs,
                           VM_ValueToSymbol(programCounter - 5));
                if (vm.breakFunction &&
                    programCounter - 5 == vm.breakFunction)
                {
                     ...
                     * debugger */
                    vm.breakCount++;
                    VM_StackTrace(programCounter, programStack);
                }
            }
#endif
            DISPATCH();
...
static void VM_LoadSymbols()
{
    ...
    while (1)
    {
        ...
        chars = strlen(token);
        sym   = Com_malloc(sizeof(*sym) + chars, VM_ALLOC_DEBUG);
        *prev = sym;
        if (!sym)
        {
            Com_Error(VM_MALLOC_FAILED,
    ...
    Com_free(mapfile.v, VM_ALLOC_DEBUG);
}
...
void VM_VmProfile_f()
{
    ...
    sorted = Com_malloc(vm.numSymbols * sizeof(*sorted), VM_ALLOC_DEBUG);
    if (!sorted)
    {
        Com_Error(VM_MALLOC_FAILED,
    ...
    Com_free(sorted, VM_ALLOC_DEBUG);
}
#else
void VM_VmProfile_f()
{
